...
async function merge(octokit, pullRequest) {
  const pr = await pullRequestState(octokit, pullRequest);
  if (pr.state === "behind") {
    ...
    logger.info("No update done due to PR state", pr.state);
    throw new NeutralExitError();
  }
}
...
async function pullRequestState(octokit, pullRequest) {
  if (pullRequest.mergeable_state) {
    ...
    return {
      mergeable: fullPullRequest.mergeable,
      state: fullPullRequest.mergeable_state
    };
  }
}
